<template>
  <nav
    class="tw:fixed tw:bottom-0 tw:z-10 tw:w-full tw:bg-neutral-99 tw:text-sm tw:font-medium tw:leading-4 tw:tracking-wider tw:dark:bg-neutral-10 tw:md:left-0 tw:md:top-0 tw:md:mt-16 tw:md:hidden tw:md:h-full tw:md:w-20"
  >
    <ul
      class="tw:flex tw:list-none tw:items-center tw:justify-around tw:gap-x-2 tw:bg-primary-40/12 tw:py-1 tw:pl-0! tw:dark:bg-primary-80/12 tw:md:h-full tw:md:flex-col tw:md:justify-center tw:md:gap-y-3 tw:md:py-0"
    >
      <TwNavigationBarButton
        :to="localePath(`/regions/${region}/`)"
        :icon="mdiHomeOutline"
        :active-icon="mdiHome"
      >
        {{ $t('app.tabHome') }}
      </TwNavigationBarButton>
      <TwNavigationBarButton
        :to="localePath(`/regions/${region}/map`)"
        :icon="mdiMapOutline"
        :active-icon="mdiMap"
      >
        {{ $t('app.tabMap') }}
      </TwNavigationBarButton>
      <TwNavigationBarButton
        :to="localePath(`/regions/${region}/table`)"
        :icon="msTableOutline"
        :active-icon="msTable"
      >
        {{ $t('app.tabTable') }}
      </TwNavigationBarButton>
      <TwNavigationBarButton
        :to="localePath(`/regions/${region}/download`)"
        :icon="mdiDownloadOutline"
        :active-icon="mdiDownload"
      >
        {{ $t('app.tabDownload') }}
      </TwNavigationBarButton>
      <TwNavigationBarButton
        v-if="adminMode"
        :to="localePath('/debug')"
        :icon="mdiTools"
      >
        Admin
      </TwNavigationBarButton>
    </ul>
  </nav>
</template>

<script>
import {
  mdiHomeOutline,
  mdiHome,
  mdiMap,
  mdiMapOutline,
  mdiDownload,
  mdiDownloadOutline,
  mdiTools,
} from '@mdi/js'
export default {
  data: () => ({
    mdiHomeOutline,
    mdiHome,
    mdiMap,
    mdiMapOutline,
    msTable:
      'M21 21v-4.65H3V21h18Zm0-6.65V9.625H3v4.725h18Zm0-6.725V3H3v4.625h18Z',
    msTableOutline:
      'M19 19v-3H5v3h14Zm0-5v-4H5v4h14Zm0-6V5H5v3h14ZM5 21q-.825 0-1.413-.587Q3 19.825 3 19V5q0-.825.587-1.413Q4.175 3 5 3h14q.825 0 1.413.587Q21 4.175 21 5v14q0 .825-.587 1.413Q19.825 21 19 21Z',
    mdiDownload,
    mdiDownloadOutline,
    mdiTools,
  }),
  computed: {
    region() {
      return (
        this.$route.params.region || this.$store.state.settings.currentRegion
      )
    },
    adminMode() {
      return this.$store.state.settings.adminMode
    },
  },
}
</script>
